    ...
    <DialogPortal hostName={portalHost}>
      <DialogOverlay>
        <DialogPrimitive.Content
          className={cn(
            'max-w-lg gap-4 border border-border web:cursor-default bg-secondary p-6 shadow-lg web:duration-200 rounded-lg',
            open
...
function DialogHeader({ className, ...props }: ViewProps) {
  return (
    <View className={cn('flex gap-1.5 text-center sm:text-left', className)} {...props} />
  );
}
...
function DialogTitle({
  ...
  return (
    <DialogPrimitive.Title
      className={cn(
        'text-lg native:text-xl text-secondary-foreground font-semibold leading-none tracking-tight',
        className
      )}
      {...props}
    />
